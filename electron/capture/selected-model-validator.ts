...
function includes(values: readonly string[], language: string): boolean {
  return values.some((value) => value.toLocaleLowerCase() === language.toLocaleLowerCase());
}
...
export async function validateSelectedModels(
  ...
  if (!includes(library.enabledTranslationLanguages, settings.translationLanguage)) {
    throw new Error("The selected translation language is not enabled.");
  }
